...
// Sales returns ("salesreturn") — goods a customer sent back against a
// shipment. Reduces what they still owe on the order that shipment was
...
async function syncSalesReturns(client) {
  ...
  try {
    ...
    if (needsFull && !isFirst && coolingDown(state)) {
      console.warn(`[ms-sync] sales_returns: no watermark yet, but a full resync already ran within the last ${FULL_SYNC_COOLDOWN_HOURS}h — skipping this tick to avoid hammering the API`);
      await upsertMeta(client, entity, { last_status: 'skipped_cooldown', last_error: null });
      return;
    }
    ...
    const since = needsFull ? `${ACCOUNT_CUTOVER_DATE} 00:00:00` : state.watermark_s;
    const filter = buildDateBoundedFilter([since ? `${filterField}>=${since}` : null]);
    const rawRows = await msAll('/entity/salesreturn', `${filter}&expand=agent&order=moment,asc`);
    ...
    const rows = rawRows.map(r => {
      ...
      return {
        ...
        sum_kopecks: Math.round(r.sum || 0),
        customer_id: hrefTail(r.agent?.meta?.href) || null, customer_name: r.agent?.name || null,
        demand_id: hrefTail(r.demand?.meta?.href) || null,
        updated_at: (r.updated || '').slice(0, 19) || null,
      };
    });
    if (!maxUpdated && rawRows.length) {
      maxUpdated = fallbackWatermark();
      console.warn(`[ms-sync] sales_returns: no updated on any row — using clock-based fallback watermark ${maxUpdated}`);
    }

    const cols = ['id', 'name', 'moment', 'sum_kopecks', 'customer_id', 'customer_name', 'demand_id', 'updated_at'];
    await batchUpsert(client, 'ms_sales_returns', cols, ['id'], rows);
    ...
    console.log(`[ms-sync] sales_returns: ${rows.length} synced (${needsFull ? 'full' : 'incremental'})`);
  } catch (e) {
    await upsertMeta(client, entity, { last_status: 'error', last_error: e.message.slice(0, 500) }).catch(() => {});
    console.error('[ms-sync] sales_returns failed:', e.message);
  }
}
...
async function runSync() {
  ...
  await withClient(pool, syncSalesReturns);
